...
const TABLE_ICONS: Record<string, typeof Users> = {
  profiles: Users, todos: CheckSquare, notes: FileText, bookmarks: Bookmark,
  projects: FolderKanban, meetings: CalendarClock, diagrams: GitBranch,
  tips: Terminal, feed_items: Rss, routines: Repeat,
};

const TABLE_LABELS: Record<string, string> = {
  profiles: "Comptes", todos: "Todos", notes: "Notes", bookmarks: "Bookmarks",
  projects: "Projets", meetings: "Meetings", diagrams: "Diagrammes",
  tips: "Tips", feed_items: "Veille", routines: "Routines",
};
      ...
      {data && (
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            {/* Signups */}
            <ChartCard title="Inscriptions (30 derniers jours)" subtitle={`${data.usersCount} comptes au total`}>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={data.signupsPerDay} margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="signups" stroke="hsl(var(--primary))" fill="url(#signups)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </ChartCard>
            ...
            <ChartCard title="Activité (30j)" subtitle="Création de contenu / jour">
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={data.activity} margin={{ top: 5, right: 8, left: -20, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="todos" stroke="#3b82f6" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="notes" stroke="#a78bfa" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="feeds" stroke="#22c55e" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </ChartCard>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie
                    ...
                    cx="50%" cy="50%" outerRadius={80} label={{ fontSize: 11 }}
                  >
            ...
            <ChartCard title="Volume par table" subtitle="Total cumulé">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={Object.entries(data.totals).map(([k, v]) => ({ name: TABLE_LABELS[k] ?? k, count: v }))}
          ...
          <div className="rounded-lg border bg-card">
            <div className="px-5 pt-4 pb-2 flex items-center justify-between">
              <h3 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Logs cron (20 derniers)</h3>
              <Badge variant="outline" className="text-[10px]">tâches planifiées</Badge>
            </div>
                        ...
                        <td className="px-3 py-2">
                          <Badge
                            ...
                            className={`h-5 text-[10px] ${
                              r.status === "succeeded" ? "border-green-500/30 text-green-500 bg-green-500/10" :
                              r.status === "failed" ? "border-red-500/30 text-red-500 bg-red-500/10" :
                              "border-amber-500/30 text-amber-500 bg-amber-500/10"
                            }`}
                          >{r.status}</Badge>
                        </td>
                        <td className="px-3 py-2 text-muted-foreground">{new Date(r.start_time).toLocaleString("fr-FR")}</td>
                        <td className="px-3 py-2 text-muted-foreground truncate max-w-xs">{r.return_message ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <div className="rounded-lg border bg-card">
              <div className="px-5 pt-4 pb-2 flex items-center justify-between">
                <h3 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Sources RSS</h3>
                <Badge variant="outline" className="text-[10px]">{data.sources.length}</Badge>
              </div>
              <div className="px-5 pb-4 space-y-2">
                {data.sources.slice(0, 12).map((s) => (
                  <div key={s.id} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <Badge variant="outline" className="h-5 text-[9px]">{s.source_type}</Badge>
                    {!s.enabled && <Badge variant="outline" className="h-5 text-[9px] text-muted-foreground">off</Badge>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-lg border bg-card">
              <div className="px-5 pt-4 pb-2 flex items-center justify-between">
                <h3 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Veille — derniers items</h3>
                <Badge variant="outline" className="text-[10px]">{data.recentFeeds.length}</Badge>
              </div>
              <div className="px-5 pb-4 space-y-2">
                {data.recentFeeds.map((f) => (
                  <div key={f.id} className="flex items-start justify-between gap-2 text-xs">
                    ...
                    <Badge
                      ...
                      style={{ borderColor: (SEV_COLORS[f.severity] ?? "#888") + "55", color: SEV_COLORS[f.severity] ?? "#888" }}
                    >{f.severity}</Badge>
                  </div>
                ))}
              </div>
